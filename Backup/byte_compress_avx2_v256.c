...
// Table Size:  256 * 8 * 2 byte = 2048 bytes = 2KB
short perm[256][8] = {0};
...
// 256 bit byte vector is composed of 32 bytes.
// We can avoid generating permute table of 128GB which is indexed by 32 bit mask
// by generating a smaller 2KB table and read and assemble 4 permute rows by using
// four byte (8 bit) slices of a 32 bit mask.

// Generate byte level permute table
// to ease direct usage of vpshufb
void gen_perm_table_byte() {
  for (int i = 0; i < 256; i++) {
     int j = 0;
     int index = 0;
     int tmp = i;
...
void compress_byte_avx2(char* res, char* src, int mask, int vlen) {
  asm volatile (
     // Prepare allones vector.
     "vpcmpeqb %%ymm2, %%ymm2, %%ymm2 \n\t"

     // Extract first 128 bit permute vector.
     ...
     "vmovdqu %%ymm2, %%ymm1 \n\t"
     "movdqu (%%rcx), %%xmm1 \n\t"
     "vpacksswb %%ymm2, %%ymm1, %%ymm1 \n\t"

     // Extract second 128 bit permute vector.
     ...
     // Assemble partial permutation mask
     // SHORT_COMPRESS_PERM_MASK = BLENDVB LO_PERM, HI_PERM, LO_PERM
     "vpblendvb %%ymm1, %%ymm3, %%ymm1, %%ymm1 \n\t"
     
     // Extract third 128 bit permute vector.
     // HI_PERM = PERM_TABLE[ INDEX >> 16]
     // HI_PERM = HI_PERM + BROADCAST 8
     "movl %2,    %%ecx   \n\t"
     "shr $0x10,  %%ecx   \n\t"
     "andl $0xFF, %%ecx   \n\t"
     ...
     "vperm2i128 $1, %%ymm3, %%ymm3, %%ymm4 \n\t"
     "vpshufb %%ymm5, %%ymm4, %%ymm4 \n\t"
     "vpshufb %%ymm5, %%ymm3, %%ymm3 \n\t"
     "vpaddb  %5, %%ymm5, %%ymm7 \n\t"
     "vpblendvb %%ymm7, %%ymm4, %%ymm3, %%ymm3 \n\t"

     // Assemble partial permutation mask
     // SHORT_COMPRESS_PERM_MASK = BLENDVB LO_PERM, HI_PERM, LO_PERM
     "vpblendvb %%ymm1, %%ymm3, %%ymm1, %%ymm1 \n\t"

     // Extract fourth 128 bit permute vector.
     // HI_PERM = PERM_TABLE[ INDEX >> 24]
     // HI_PERM = HI_PERM + BROADCAST 8
     "movl %2,    %%ecx   \n\t"
     "shr $0x18,  %%ecx   \n\t"
     "andl $0xFF, %%ecx   \n\t"
    ...
    : "cc", "%ymm1", "%ymm2", "%ymm3", "%ymm4", "%ymm5", "%ymm6", "%ymm7", "%ebx", "%ecx", "%edx", "%eax", "%rsi" 
  );
} 
...
void print_vec(char* vec, int vlen) {
  printf("[ ");
...
int main(int argc, char * argv[]) {
  ...
  char res [32];
  compress_byte_avx2(res, src, mask, 32); 
  print_vec(src, 32);
  print_mask(mask);
  print_vec(res, 32);
  return 0;
