...
*/
  ...
  asm volatile (
      ...
      "vpsraq $63, %%zmm1, %%zmm4     \n\t"
      // find the result lanes corresponding to opposite sign inputs whose sign does not match with first input sign.
      "vpcmpq $4, %%zmm5, %%zmm4, %%k2 %{%%k1} \n\t"
      // replace result lanes corresponding to opposite sign inputs where sign flipped with saturated value (Integer.MIN_VALUE for -ve numbers else Integer.MAX_VALUE)
      "vmovdqu64 %3, %%zmm7 \n\t"
      "vmovdqu64 %4, %%zmm8 \n\t"
      // if value at flipped sign result lane is +ve then replacit it by MIN_VALUE else with MAX_VALUE
      "vpcmpeqq %%zmm5, %%zmm6, %%k3 %{%%k2}  \n\t"
      "kandnw %%k2, %%k3, %%k4              \n\t"
      "vpblendmq %%zmm8, %%zmm3 , %%zmm3 %{%%k3} \n\t"
      "vpblendmq %%zmm7, %%zmm3 , %%zmm3 %{%%k4} \n\t"
      "vmovdqu64 %%zmm3, %0           \n\t"
     : "=m"(res[0])
  ...
  return printf("ACTUAL {res[0], res[1], res[2], res[3]} = {%ld, %ld, %ld, %ld}\n", res[0], res[1], res[2], res[3]);
}
