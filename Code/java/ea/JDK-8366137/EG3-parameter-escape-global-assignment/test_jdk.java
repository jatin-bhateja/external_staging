...
public class test_jdk {
    public static Point sobj = null;

    public static float compute_angle_with_x_axis(Point obj) {
        sobj = obj; 
        float distance = sobj.compute_distance_from_origin();
        return (float)Math.acos(sobj.x / distance);
    } 
 
    public static float compute_angle_with_y_axis(Point obj) {
        sobj = obj;
        float distance = sobj.compute_distance_from_origin();
        return (float)Math.acos(sobj.y / distance);
    } 
 
    public static float compute_angle_with_z_axis(Point obj) {
        sobj = obj;
        float distance = sobj.compute_distance_from_origin();
        return (float)Math.acos(sobj.z / distance);
    } 
 
    public static float micro() {
        Point obj = new Point(1.0f, 2.0f, 3.0f);
