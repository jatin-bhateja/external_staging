class dep {
 ...
 static void test1() {
     for (int i = 20; i < 100; i++) { 
         //iArr[i] = iArr[i + 4]; // intra-stmt L->S (forward store)
         iArr[i - 1] = iArr[i - 11] * 10; // inter-stmt L->S(forward store) , intra-stmt (S->L future loads) 
         fArr[i - 1] = (float)iArr[i] * i; // inter-stmt L->S(forward store) , intra-stmt (S->L future loads) 
     }
 }
