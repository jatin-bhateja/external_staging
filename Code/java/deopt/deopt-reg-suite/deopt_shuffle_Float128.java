...
class deopt_shuffle_Float128 {
  ...
  public static int micro(int [] arr, int ctr) {
      VectorShuffle<?> shuffle = VectorShuffle.fromArray(FloatVector.SPECIES_128, arr, 0);
      if ((ctr % 2500) == 2000) {
          leaf(shuffle);
      }
      return shuffle.laneSource(ctr & (FloatVector.SPECIES_128.length() - 1));
  }
