...
class deopt_shuffle_Short256 {
  ...
  public static int micro(int [] arr, int ctr) {
      VectorShuffle<?> shuffle = VectorShuffle.fromArray(ShortVector.SPECIES_256, arr, 0);
      if ((ctr % 2500) == 2000) {
          leaf(shuffle);
      }
      return shuffle.laneSource(ctr & (ShortVector.SPECIES_256.length() - 1));
  }
