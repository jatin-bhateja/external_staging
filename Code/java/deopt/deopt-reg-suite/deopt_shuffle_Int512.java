...
class deopt_shuffle_Int512 {
  ...
  public static int micro(int [] arr, int ctr) {
      VectorShuffle<?> shuffle = VectorShuffle.fromArray(IntVector.SPECIES_512, arr, 0);
      if ((ctr % 2500) == 2000) {
          leaf(shuffle);
      }
      return shuffle.laneSource(ctr & (IntVector.SPECIES_512.length() - 1));
  }
