...
class deopt_shuffle_Long128 {
  ...
  public static int micro(int [] arr, int ctr) {
      VectorShuffle<?> shuffle = VectorShuffle.fromArray(LongVector.SPECIES_128, arr, 0);
      if ((ctr % 2500) == 2000) {
          leaf(shuffle);
      }
      return shuffle.laneSource(ctr & (LongVector.SPECIES_128.length() - 1));
  }
