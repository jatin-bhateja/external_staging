...
class deopt_shuffle_Short64 {
  ...
  public static int micro(int [] arr, int ctr) {
      VectorShuffle<?> shuffle = VectorShuffle.fromArray(ShortVector.SPECIES_64, arr, 0);
      if ((ctr % 2500) == 2000) {
          leaf(shuffle);
      }
      return shuffle.laneSource(ctr & (ShortVector.SPECIES_64.length() - 1));
  }
