...
class deopt_shuffle_Byte256 {
  ...
  public static int micro(int [] arr, int ctr) {
      VectorShuffle<?> shuffle = VectorShuffle.fromArray(ByteVector.SPECIES_256, arr, 0);
      if ((ctr % 2500) == 2000) {
          leaf(shuffle);
      }
      return shuffle.laneSource(ctr & (ByteVector.SPECIES_256.length() - 1));
  }
