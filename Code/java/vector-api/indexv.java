...
class indexvec {
   ...
   public static byte micro(byte [] res, byte [] arr, int limit, int ctr) {
      for(int i = 0;  i < limit; i += SPECIES.length()) {
          VectorMask<Byte> m = SPECIES.indexInRange(i, limit);
          ByteVector.fromArray(SPECIES, arr, i, m)
                    .intoArray(res, i, m);
      }
      return res[ctr];
   }
...
*/
      byte [] res = new byte[1027];
      byte [] arr = new byte[1027];
      long val = 0;
      for (int i = 0; i < 100000; i++) {
         val += micro(res, arr, 1024, i & 1023);
      }
      long t1 = System.currentTimeMillis();
      for (int i = 0; i < 100000; i++) {
         val += micro(res, arr, 1024, i & 1023);
