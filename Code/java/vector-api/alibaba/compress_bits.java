...
public class compress_bits {
   ...
   public static int compressBits(int a, int mask) {
        return 1;//Integer.compress(a, mask);
   }
   ...
   public static void vector_compressBits(int [] r, int [] a, int [] mask, int pos) {
    ...
    public static void vector_compressBits_VAPI(int [] r, int [] a, int [] mask, int pos) {
        IntVector aV = IntVector.fromArray(SPECIES, a, pos);
        IntVector maskV = IntVector.fromArray(SPECIES, mask, pos);
        aV.lanewise(VectorOperators.NOT, maskV)
          .intoArray(r, pos);
    }
    ...
    public static int vector_runnerVAPI(int [] r , int [] a, int [] mask, int ctr) {
      int i = 0;
      for(; i < SPECIES.loopBound(a.length); i+=SPECIES.length()) {
        vector_compressBits_VAPI(r, a, mask, i);
      }
      /*for(; i < a.length; i++) {
        r[i] = compressBits(a[i], mask[i]);
      }*/
   ...
   public static void main(String [] args) {
      ...
      if (algo == 0) {
        ...
        int ret = 0;
        for(int i = 0; i < 500000; i++) {
           ret += vector_runnerVAPI(res, arr, mask, i & (len-1)); 
        }
        ...
        System.out.println("[VectorVAPI Time] " + (t2-t1) + " ms   [Res] " + ret);
        for(int i = 0; i < SPECIES.length(); i++) {
           System.out.println("r["+i+"] = " + res[i]); 
        }
