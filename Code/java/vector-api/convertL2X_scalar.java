...
class convert {
  public static <E extends Number> Number zeroValue(E to) {
  ...
  public static <E extends Number, F extends Number> Number convertValue(E from,
                                                                         F to) {
    if (to.getClass().equals(Byte.class))
  ...
  public static <E extends Number, F extends Number> void
  ...
  public static void workload_convert_expand(byte[] arr, long[] res, int part) {
    ...
    Byte[] boxed_arr = getBoxedByteArray(arr);
    Long[] boxed_res = LongStream.of(res).boxed().toArray(Long[] ::new);
    Long[] boxed_ref = LongStream.of(ref).boxed().toArray(Long[] ::new);

    convert.<Byte, Long>convert_scalar_expanding(
        boxed_arr, boxed_ref, ByteVector.SPECIES_256.length(), LongVector.SPECIES_256.length() , 0, 0, part);
