...
class Utf8Validator {
    ...
    static void validate_opt(byte[] buffer, int length) {
        ...
        if ((errors | previousIncomplete) != 0) {
            throw new AssertionError("The input is not valid UTF-8");
        }
    }
    ...
    public static void main(String[] args) {
        //byte [] str = "Some random string 𑇄𑇄𑇄𑇄".getBytes();
        byte [] str = "[1, 2, 3]".getBytes();
        for (int i = 0; i < 10000000; i++) {
            Utf8Validator.validate(str, str.length);
        }
        ...
        System.out.println("[time baseline] " + (t2 - t1) + " ms  PASS");

        for (int i = 0; i < 10000000; i++) {
            Utf8Validator.validate_opt(str, str.length);
        }
        ...
        System.out.println("[time optimized] " + (t2 - t1) + " ms  PASS");
