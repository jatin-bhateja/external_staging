...
__attribute__((noinline))
uint8_t array_copy_rep_fwd( uint8_t *dst, uint8_t *src, long len, int ptype) {
   int ret; 
   if (ptype == 1) {
   asm volatile (
     "mov    %0 , %%rdi          \n\t"
...
__attribute__((noinline))
uint8_t array_copy_rep_back( uint8_t *dst, uint8_t *src, long len, int ptype) {
   int ret; 
   if (ptype == 1) {
   asm volatile (
     "mov    %0 , %%rdi          \n\t"
...
int main(int argc, char * argv[]) {
  ...
  if (atoi(argv[3]) == 1) {
    ...
    for (int i = 0 ; i < ITERS ; i++) {
      dst = dst_start;
      res += array_copy_rep_fwd(dst , src, mem_blk_size, 1);  
      iter_ctr++;
    }
  } else {
    uint8_t* dst_start = dst;
    for (int i = 0 ; i < ITERS ; i++) {
      dst = dst_start;
      res += array_copy_rep_back(dst , src, mem_blk_size, 1);  
      iter_ctr++;
    }
  }
