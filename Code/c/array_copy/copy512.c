...
int array_copy_512( uint8_t *dst , uint8_t * src, int src_length, int dst_length, int bytes, long mask) {
   ...
   while(bytes > 64) {
     asm volatile (
       ...
       "movq     %0 , %%rsi             \n\t"
       "kmovq    %2   , %%k2            \n\t"
       "vmovdqu64 (%%rdx) , %%zmm0 {{%%k2}} \n\t" 
       "vmovdqu64 %%zmm0 , (%%rsi) {{%%k2}}   \n\t" 
      : "=m"(dst) : "m"(src) , "r"(mask): "%zmm0" , "%k2" ,"%rsi", "%rdx" ); 
     bytes -= 64;
     dst += 64;
     src += 64;
   }
...
int main(int argc, char * argv[]) {
  ...
  for (int i = 0 ; i < ITERS ; i++) {
    res += array_copy_512(((uint8_t*)dst) , ((uint8_t*)src), MEM_SIZE-512, MEM_SIZE-512, MEM_SIZE-512, mask);  
    iter_ctr++;
  }
