...
__attribute__((noinline))
long gather_move_byte_avx3(char* src, int* index, int length) {
  long res = 0;
  asm volatile(
    "movq %1, %%rsi \n\t"
    "movq %2, %%rdi \n\t"
    ...
    : "%rax", "%rsi", "%zmm0", "%ecx", "%rdi", "%zmm1", "%zmm2", "%zmm3", "%zmm4"
  );
  return res;
}
...
int main() {
  ...
  res = 0;
  for (int i = 0; i < 90000000; i++) {
      res += gather_move_byte_avx3(source, index, 64);
  }
