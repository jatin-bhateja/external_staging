...
// Get the register mnemonic based on the REX.VVVV, ModRM.REG, and ModRM.RM fields (XMM registers)
const char* X86_REX_Encoding::get_XMM(uint8_t reg) {
  switch (reg) {
    case 0:
        return "%xmm0";
    case 1:
...
// Get the register mnemonic based on the REX.VVVV, ModRM.REG, and ModRM.RM fields (GPRs)
const char* X86_REX_Encoding::get_GPR(uint8_t reg) {
  switch (reg) {
    case 0:
       return "%rax";
    case 1:
...
// Get the legacy escape sequence corresponding to REX.MAP_SELECT
const char* X86_REX_Encoding::get_legacy_escape_sequence(uint8_t map_select) {
  switch (map_select) {
    case 1:
      return "0F";
    case 2:
...
uint8_t X86_REX_Encoding::get_base_reg_bit() {
  assert(modrm.MODE  == 0x3 && "Cannot hold base register");
  return rex_B; 
}

uint8_t X86_REX_Encoding::get_index_reg_bit() {
  assert(modrm.MODE  == 0x3 && "Cannot hold base register");
  return rex_X; 
}

uint8_t X86_REX_Encoding::get_reg_reg_bit() {
  assert(modrm.MODE  == 0x3 && "Cannot hold base register");
  return rex_R; 
}

bool X86_REX_Encoding::is_64bit_operand() {
  return rex_W; 
}

const char* X86_REX_Encoding::get_legacy_simd_prefix(uint8_t pp) {
  switch (pp) {
    case 1:
      return "66";
    case 2:
...
void printEncoding(X86_REX_Encoding instr) {
  DUMP("REX.B   : %x", instr.rex_B);
  DUMP("REX.X   : %x", instr.rex_X);
  DUMP("REX.R   : %x", instr.rex_R);
  DUMP("REX.N4   : %x", instr.rex_N4);
  ...
  // REX.R, REX.B and REX.X bits are used as it is without complimenting the bit..
...
void decodeEncoding(const uint8_t* encodings, int length) {
  X86_REX_Encoding instr;
  if (length == 0) {
    return;
  }
  memcpy(((char*)&instr), encodings, 1);
  if (encodings[1] == 0x0F) {
    int un = encodings[2] & 0xF0;
    assert(un == 0xA0 || un == 0x70 || un == 0x40  && "Map 1 opcode must be 0x4*, 0x7* or 0xA*");
    DUMP("Secondary Opcode Map Instruction");
    memcpy(((char*)&instr) + 1, encodings + 2, 2);
  } else {
    DUMP("Primiary Opcode Map Instruction");
