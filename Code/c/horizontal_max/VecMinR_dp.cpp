...
*/
__attribute__((noinline))
double vminpd256(__m256d arg) {
  __m256d H128_arg = _mm256_permute2f128_pd(arg, arg, 0x1);
  __m256d R1 = _mm256_min_pd(arg, H128_arg);
  __m256d H64_R1 = _mm256_shuffle_pd(R1, R1, 0x1);
  __m256d R2 = _mm256_min_pd(R1, H64_R1);
  return R2[0];
}

__attribute__((noinline))
double vminpd128(__m128d arg) {
  __m128d H64_R1 = _mm_shuffle_pd(arg, arg, 0x1);
  __m128d R2 = _mm_min_pd(arg, H64_R1);
  return R2[0];
}
...
int main() {
  ...
  double res256  =  vminpd256(arg1);
  printf("\nRes256 = %f\n", res256);

  __m128d arg2 =  _mm_load_pd(mem1);
  print_vec<__m128d>(arg2,2);
  double res128  =  vminpd128(arg2);
  printf("\nRes128 = %f\n", res128);
