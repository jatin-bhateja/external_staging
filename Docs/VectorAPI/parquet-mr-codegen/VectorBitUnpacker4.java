...
public class VectorBitUnpacker4 {
  ...
  static int [] perm_table0 = {
     ...
     12,63,12,63,14,63,14,63,
     16,63,16,63,18,63,18,63,
     20,63,20,63,22,63,22,63,
     24,63,24,63,26,63,26,63,
     28,63,28,63,30,63,30,63, 
  };
  // Left shift tables.
  // Right shift tables.
  ...
  public static final VectorMask<Byte> input_mask = VectorMask.fromLong(B128,  65535L);
  ...
  public static void scalar_packing(byte [] in , int [] out, int ctr) {
     for(int i = 0, j = 0 ; i < in.length-4 ; i+=4, j+= 8) {
  ...
  public static void vector_packing(byte [] in , int [] out, int ctr) {
     int i = 0;
     int j = 0;
     for(; i < in.length-16; i+=16, j+=32) {
        VectorBitUnpacker4.unpack8Values_vec(in, i, out, j);
     }
     for(; i < in.length-4; i+=4, j+= 8) {
        packer.unpack8Values(in, i, out, j);
     }
  }

  public static final BytePacker packer =  ByteBitPackingLE.factory.newBytePacker(4); 
