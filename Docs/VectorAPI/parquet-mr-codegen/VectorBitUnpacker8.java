...
public class VectorBitUnpacker8 {

  // Permutation tables.
  static int [] perm_table0 = {
     0,63,2,63,4,63,6,63,
     8,63,10,63,12,63,14,63,
     16,63,18,63,20,63,22,63,
     24,63,26,63,28,63,30,63,
     32,63,34,63,36,63,38,63,
     40,63,42,63,44,63,46,63,
     48,63,50,63,52,63,54,63,
     56,63,58,63,60,63,62,63, 
  };
  // Left shift tables.
  // Right shift tables.
  // AndMask tables.
  ...
  public static final VectorMask<Byte> input_mask = VectorMask.fromLong(B256,  4294967295L);
  
  public static final VectorMask<Short> or_mask = VectorMask.fromLong(S512 ,4294967295L);
  ...
  public static void unpack8Values_vec(final byte[] in, final int inPos, final int[] out, final int outPos) {
     ...
     tempRes0  = tempRes0.lanewise(VectorOperators.OR, tempRes0, or_mask);
  ...
  public static void scalar_packing(byte [] in , int [] out, int ctr) {
     for(int i = 0, j = 0 ; i < in.length-8 ; i+=8, j+= 8) {
  ...
  public static void vector_packing(byte [] in , int [] out, int ctr) {
     int i = 0;
     int j = 0;
     for(; i < in.length-32; i+=32, j+=32) {
        VectorBitUnpacker8.unpack8Values_vec(in, i, out, j);
     }
     for(; i < in.length-8; i+=8, j+= 8) {
        packer.unpack8Values(in, i, out, j);
     }
  }

  public static final BytePacker packer =  ByteBitPackingLE.factory.newBytePacker(8); 
  public static final int SIZE = 512;

  public static void main(String [] args) {
      byte [] in = new byte[SIZE*1];
