...
public class VectorBitUnpacker16 {
  ...
  public static void scalar_packing(byte [] in , int [] out, int ctr) {
     for(int i = 0, j = 0 ; i < in.length-16 ; i+=16, j+= 8) {
  ...
  public static void vector_packing(byte [] in , int [] out, int ctr) {
     int i = 0;
     int j = 0;
     for(; i < in.length-16; i+=16, j+=8) {
        VectorBitUnpacker16.unpack8Values_vec(in, i, out, j);
     }
  }

  public static final BytePacker packer =  ByteBitPackingLE.factory.newBytePacker(16); 
